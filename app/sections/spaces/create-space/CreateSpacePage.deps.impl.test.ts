...
test('maps create space request and response', async () => {
  const fetch = vi
    ...
    .mockResolvedValue(new Response('42'))
  const deps = createCreateSpacePageDeps(
    createApiClient({ baseUrl: 'https://api.test', fetch }),
  )

  assert.deepEqual(
    await deps.create({ color: '#fff', key: 'product', name: 'Product' }),
    { ok: true, value: { spaceKey: 'product' } },
  )
