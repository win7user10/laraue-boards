...
const emit = defineEmits<{
  changeOrganization: []
  clearError: []
  loadSpaces: [organizationId: string]
  moveBoards: [input: { boardIds: string[]; destinationSpaceId: string }]
  moveSpaces: [input: { destinationOrganizationId: string; spaceIds: string[] }]
}>()
...
const changeOrganization = () => {
  boardSpace.value = ''
  emit('changeOrganization')
}
const loadSpaces = () => {
  if (!props.loadingSpaces && props.destinationSpaces.length === 0) {
    emit('loadSpaces', boardOrganization.value)
  }
}
