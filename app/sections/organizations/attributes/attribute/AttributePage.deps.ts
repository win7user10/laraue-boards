...
export type Attribute = {
  color: string
  data: { listValues: Array<{ id: string; name: string }>; type: 'list' } | { type: 'text' }
  id: string
  name: string
}
...
export type UpdateAttributeInput = {
  color: string
  data: { listValues: Array<{ id: null | string; name: string }>; type: 'list' } | { type: 'text' }
  id: string
  name: string
}
...
export type AttributePageDeps = {
  ...
  update: (input: UpdateAttributeInput) => Promise<Result<void, ChangeAttributeFailure>>
  view: (input: {
    attributeId: string
    signal?: AbortSignal
